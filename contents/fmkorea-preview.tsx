...
const YT_RE = /(?:youtube\.com\/(?:watch\?[^&\s]*?v=|shorts\/)|youtu\.be\/)([A-Za-z0-9_-]{11})/;
...
const makeVideoWrap = (iframe: HTMLIFrameElement): HTMLElement => {
  ...
  wrap.style.cssText = "width:100%;aspect-ratio:16/9;margin:8px 0;border-radius:8px;overflow:hidden";
...
const makeEmbed = (url: string): HTMLElement | null => {
  ...
  if (tw) {
    ...
    iframe.style.cssText = "width:100%;max-width:550px;border:0;border-radius:12px;margin:8px 0;display:block;min-height:280px";
    return iframe;
  }
  ...
  if (ig) {
    ...
    iframe.style.cssText = "width:100%;max-width:400px;border:0;border-radius:8px;margin:8px 0;display:block;min-height:480px";
    return iframe;
  }
  return null;
};
...
const embedMedia = (root: Element) => {
  ...
  const collectTextNodes = (el: Element): Text[] => {
    ...
    for (const child of Array.from(el.childNodes)) {
      ...
      else if (child.nodeType === 1 && !["IFRAME", "SCRIPT", "STYLE"].includes((child as Element).tagName))
                ...
                <div className="flex flex-col items-center gap-4 py-8 border-y border-gray-100">
                  ...
                  <div className="flex gap-3">
                    <button
                      ...
                      className="px-8 py-2.5 rounded-full border-2 border-blue-200 text-blue-500 hover:bg-blue-50 font-semibold text-sm transition-colors"
                    >
                      추천
                    </button>
                    <button
                      onClick={() => handleVote("down")}
                      className="px-8 py-2.5 rounded-full border-2 border-gray-200 text-red-400 hover:bg-red-50 font-semibold text-sm transition-colors"
                    >
                      비추천
                    </button>
                  </div>
                  ...
                  {post.totalCommentPages > 1 && (
                    <div className="flex items-center justify-center gap-1 py-3">
                      <button
                        ...
                        disabled={post.commentPage === 1 || commentPageLoading}
          ...
          <ul>
            {[
              ...
              },
              {
                ...
                onClick: () => {
                  try {
                    (window as any).blind_click(null, memberPopup.memberSrl, 0);
                  } catch {}
                  setMemberPopup(null);
                },
              },
              {
                ...
                onClick: () => {
                  try {
                    (window as any).blind_click(null, memberPopup.memberSrl, 1);
                  } catch {}
                  setMemberPopup(null);
                },
              },
...
function CommentItem({
  ...
  return (
    <div
      ...
      className={`flex gap-3 py-4 px-2 border-b border-gray-200 last:border-0 ${
        comment.isReply
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2 mb-1.5">
          ...
          <button
            onClick={(e) => onAuthorClick(comment.memberSrl, comment.author, e)}
